...
const priorityColor = (p: string) =>
    p === 'High' ? { bg: '#fee2e2', fg: '#be123c', dot: '#f43f5e' } :
        p === 'Medium' ? { bg: '#ffedd5', fg: '#c2410c', dot: '#f97316' } :
            { bg: '#eef0f6', fg: '#5b6382', dot: '#94a3c0' }

const scoreColor = (s: number) =>
    s >= 80 ? { bg: '#d1fae5', fg: '#047857' } :
        s >= 50 ? { bg: '#ffedd5', fg: '#c2410c' } :
            { bg: '#eef0f6', fg: '#5b6382' }

const statusColor = (s: string) =>
    s === 'New' ? { bg: '#e0f7fb', fg: '#0e7f96' } : { bg: '#d1fae5', fg: '#047857' }
...
export default function DashboardClient({ initialLeads }: { initialLeads: Lead[] }) {
    ...
    return (
        ...
        .nav-item:hover:not(.nav-disabled) { background: #f5f7ff !important; }
            ...
            <main className="app-main" style={{ flex: 1, minWidth: 0, padding: '28px 36px 48px', maxWidth: '1120px', margin: '0 auto' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '26px', gap: '16px' }}>
                    <div style={{ position: 'relative', flex: 1, maxWidth: '360px' }}>
                        <Search size={16} color="#94a3c0" style={{ position: 'absolute', left: '14px', top: '50%', transform: 'translateY(-50%)' }} />
                        <input
                            value={searchQuery}
                            ...
                            style={{
                                ...
                                background: 'white', padding: '10px 14px 10px 40px', fontSize: '13.5px', outline: 'none', color: '#12162b'
                            }}
                        />
                    </div>
                ...
                <div className="stat-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '20px' }}>
                    {[
                        ...
                        <div key={s.label} className="stat-card fade-up" style={{
                            ...
                            background: 'white', borderRadius: '18px', padding: '20px', position: 'relative',
                            boxShadow: '0 12px 32px rgba(15,23,42,0.06)',
                            display: 'flex', justifyContent: 'space-between', alignItems: 'center'
                        }}>
                            <div>
                                <p style={{ fontSize: '11.5px', color: '#94a3c0', fontWeight: 600, marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.03em' }}>{s.label}</p>
                ...
                <div className="fade-up" style={{
                    ...
                    <p style={{ fontSize: '11.5px', color: '#94a3c0', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '4px' }}>
                        Last 7 days
                    </p>
                    <p className="font-display" style={{ fontSize: '16px', fontWeight: 700, marginBottom: '18px' }}>
                        New leads over time
                    </p>
                    ...
                    <svg viewBox={`0 0 ${chartW} ${chartH + 24}`} style={{ width: '100%', height: 'auto', overflow: 'visible' }}>
                        ...
                        {points.map((p, i) => (
                            <g key={i}>
                                ...
                                <text x={p.x} y={chartH + 20} textAnchor="middle" fontSize="11" fill="#94a3c0" fontFamily="-apple-system, sans-serif">
                                    {p.label}
                                </text>
                            </g>
                        ))}
                ...
                <div className="fade-up" style={{
                    ...
                    <p className="font-display" style={{ fontSize: '16px', fontWeight: 700, marginBottom: '22px' }}>
                            ...
                            <div key={stage.key} style={{ display: 'flex', alignItems: 'center', flex: i < pipelineStages.length - 1 ? 1 : 'none' }}>
                                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px', minWidth: '76px' }}>
                                    ...
                                    <div style={{ textAlign: 'center' }}>
                                        <p className="font-data" style={{ fontSize: '22px', fontWeight: 700, lineHeight: 1 }}>{stage.count}</p>
                                        <p style={{ fontSize: '12px', color: '#5b6382', marginTop: '6px' }}>{stage.label}</p>
                                    </div>
                                </div>
                                {i < pipelineStages.length - 1 && (
                                    <div style={{ position: 'relative', height: '1px', flex: 1, margin: '0 14px', background: '#e7e9f5', overflow: 'hidden', transform: 'translateY(-20px)' }}>
                ...
                <div className="fade-up" style={{
                    ...
                    <div style={{
                        padding: '22px 26px', borderBottom: '1px solid #f1f2f6',
                        display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px'
                    }}>
                        <h2 className="font-display" style={{ fontWeight: 700, fontSize: '18px', letterSpacing: '-0.01em' }}>All Leads</h2>
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            ...
                            <tbody>
                                {filteredAndSortedLeads.length > 0 ? filteredAndSortedLeads.map((lead) => {
                                    ...
                                    return (
                                        <tr key={lead.id} className="lead-row" style={{ borderTop: '1px solid #f4f5f8' }}>
                                            <td style={tdStyle}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                                    ...
                                                    <div>
                                                        ...
                                                        <div style={{ fontSize: '13px', color: '#94a3c0' }}>{lead.email}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td style={tdStyle}>
                                                <div style={{ fontSize: '14px', color: '#334155', fontWeight: 500 }}>{lead.business_type}</div>
                                                <div style={{ fontSize: '12px', color: '#94a3c0' }}>Budget: {lead.budget}</div>
                                            </td>
                                            <td style={tdStyle}>
                                            ...
                                            <td style={{ ...tdStyle, textAlign: 'right' }}>
                                                <button
                                                    ...
                                                    className="view-btn"
                                                    style={{
                                                        color: '#4f46e5', background: '#eef1ff', border: 'none',
                                    ...
                                    <tr>
                                        <td colSpan={6} style={{ padding: '64px', textAlign: 'center', color: '#94a3c0' }}>
                                            <Search size={32} style={{ margin: '0 auto 12px', display: 'block', color: '#cbd5e1' }} />
                ...
                <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
                    ...
                    <div style={{
                        ...
                        <div style={{ padding: '24px 28px', overflowY: 'auto' }}>
                            ...
                            <div style={{ ...sectionStyle, background: '#fafbfe', border: '1px solid #f1f2f6' }}>
                            ...
                            <div style={{ ...sectionStyle, background: '#eef1ff', border: '1px solid #e0e5ff' }}>
                                <div style={sectionHeader}>
                                    <Wand2 size={14} color="#4f46e5" />
                                    <h4 style={{ ...sectionTitle, color: '#4338ca' }}>AI Recommended Action</h4>
                                </div>
                                <p style={{ fontSize: '14px', color: '#3730a3', lineHeight: 1.6 }}>
                                    {selectedLead.recommended_action || 'No recommended action.'}
                                </p>
                            </div>

                            <div style={{ ...sectionStyle, background: 'white', border: '1px solid #eef0f4', marginBottom: 0 }}>
                                <div style={sectionHeader}>
                                    <Mail size={14} color="#64748b" />
                                    <h4 style={sectionTitle}>Draft Follow-up Email</h4>
                                </div>
                                <div style={{
                                    fontSize: '13.5px', color: '#334155', whiteSpace: 'pre-wrap',
                                    fontFamily: 'ui-monospace, SFMono-Regular, monospace', lineHeight: 1.7,
                                    background: '#fafbfe', padding: '16px', borderRadius: '12px', border: '1px solid #f1f2f6'
                                }}>
                        ...
                        <div style={{
                            padding: '18px 28px', borderTop: '1px solid #f1f2f6', background: '#fafbfe',
                            display: 'flex', justifyContent: 'flex-end', gap: '10px', borderRadius: '0 0 24px 24px'
                        }}>
                            <button onClick={() => setSelectedLead(null)} style={{
                                padding: '11px 20px', borderRadius: '12px', border: '1px solid #e7e9f5',
                                background: 'white', cursor: 'pointer', fontSize: '14px', fontWeight: 600, color: '#475569'
                            }}>
                                Cancel
                            </button>
                            <button
...
const thStyle: React.CSSProperties = {
    ...
    color: '#94a3c0', textTransform: 'uppercase', letterSpacing: '0.05em'
}
...
const sectionTitle: React.CSSProperties = {
    fontSize: '11.5px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', color: '#475569'
}
