...
export default function AnalyticsClient({ leads }: { leads: Lead[] }) {
    ...
    if (total === 0) {
        return (
            <div className="fade-up" style={{ ...cardStyle, textAlign: 'center', color: '#94a3c0', padding: '48px' }}>
        ...
        <div>
            <div className="stat-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '20px' }}>
                ...
                <div className="stat-card fade-up" style={{ ...cardStyle, animationDelay: '160ms' }}>
                    ...
                    <p style={{ fontSize: '12px', color: '#94a3c0', marginTop: '2px' }}>{sent} of {total} sent</p>
                </div>
            ...
            <div className="fade-up" style={{ ...cardStyle, animationDelay: '320ms' }}>
                <p className="font-display" style={{ fontSize: '16px', fontWeight: 700, marginBottom: '20px' }}>
                    Score distribution
                </p>
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: '20px', height: '160px' }}>
                    {scoreCounts.map((b) => (
                        <div key={b.label} className="bar-col" style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', height: '100%', justifyContent: 'flex-end' }}>
                            <span className="font-data" style={{ fontSize: '13px', fontWeight: 700, marginBottom: '6px' }}>{b.count}</span>
                            ...
                            <span style={{ fontSize: '12px', color: '#5b6382', marginTop: '10px' }}>{b.label}</span>
                        </div>
                    ))}
                </div>
            </div>
                    ...
                    {priorityCounts.map((p) => (
                        <div key={p.label}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                                <span style={{ fontSize: '13.5px', fontWeight: 600, color: '#334155' }}>{p.label}</span>
                                <span className="font-data" style={{ fontSize: '13px', fontWeight: 700 }}>{p.count}</span>
                            </div>
                            <div style={{ width: '100%', height: '8px', borderRadius: '999px', background: '#f1f3fa', overflow: 'hidden' }}>
